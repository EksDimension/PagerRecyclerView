package com.eks.pagingrecyclerview.paginglayoutmanager;
...
public class PagerGridLayoutManager extends RecyclerView.LayoutManager
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State
        ...
        if (result < 0) {// 左
            // 上一页的终点X值(换算为单页)
            float lastPageFinalX = ((getPageIndex() + 1) * getUsableWidth()) - getPageIndex() * getUsableWidth();
            // 偏移量(换算为单页)
            int offsetX = mOffsetX - getPageIndex() * getUsableWidth();
            /*Log.i(TAG,
                    "当前页码:" + getPageIndex() +
                            " 偏移量" + offsetX +
                            " 偏移百分比" + (offsetX / lastPageFinalX));*/
            Log.i("233",offsetX+"   "+lastPageFinalX);
            if (mOnPageChangeCallback != null) {
                mOnPageChangeCallback.onPageScrolled(getPageIndex(), (offsetX / lastPageFinalX), offsetX);
            }
        } else if (result > 0) {// 右
            //终点X
            float finalX = getUsableWidth();
            // 本页偏移量
            ...
            /*Log.i(TAG,
                    "当前页码:" + getPageIndex() +
                            " 偏移量" + offsetX +
                            " 偏移百分比" + (offsetX / finalX));*/
            if (mOnPageChangeCallback != null) {
                mOnPageChangeCallback.onPageScrolled(getPageIndex(), (offsetX / finalX), offsetX);
            }
        }
    ...
    private int getPageIndex() {
        ...
        if (canScrollVertically()) {
            ...
            if (mOffsetY <= 0 || pageHeight <= 0) {
                ...
                pageIndex = mOffsetY / pageHeight;
                if (mOffsetY % pageHeight > pageHeight) {
                    pageIndex++;
                }
            }
        } else {
            ...
            if (mOffsetX <= 0 || pageWidth <= 0) {
                ...
                pageIndex = mOffsetX / pageWidth;
                if (mOffsetX % pageWidth > pageWidth) {
                    pageIndex++;
                }
            }
        }
